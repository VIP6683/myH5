...
function syncVisibleListCount({ replace = false, revealNew = 0 } = {}) {
	if (replace) {
		visibleListCount.value = Math.min(LINE_TASK_LIST_PAGE_SIZE, patches.value.length);
		return;
	}
...
export function findLineTaskListPatch(patchId) {
...
export function useLineTaskList() {
	...
	async function loadMoreLineTaskList({ revealInList = 0 } = {}) {
	...
	async function revealMoreListRows() {
		...
		if (hiddenCount > 0) {
			syncVisibleListCount({
				revealNew: Math.min(LINE_TASK_LIST_PAGE_SIZE, hiddenCount)
			});
			return null;
		}

		return loadMoreLineTaskList({ revealInList: LINE_TASK_LIST_PAGE_SIZE });
	}

	function resetLineTaskList() {
		resetPatchesState();
	...
	return {
		...
		loadingMore,
		loadLineTaskList,
		loadMoreLineTaskList,
		revealMoreListRows,
		findLineTaskListPatch,
		resetLineTaskList
	};
}
