...
const bindPageLifecycleRefresh = (map) => {
	...
	const onWindowFocus = () => refreshMapViewport();
...
const waitMapLoaded = (map) => {
	...
	const readyTasks = [];
	if (map?.readyPromise) {
		readyTasks.push(map.readyPromise);
	}
	readyTasks.push(waitProvinceMaskReady());

	Promise.all(readyTasks)
		.then(() => {
			if (token === loadWaitToken) finishMapLoading(map);
		})
		.catch((error) => {
			console.warn('Map or province mask failed to load.', error);
			if (token === loadWaitToken) finishMapLoading(map);
		});
};
...
const initMap = () => {
	if (isMapCreated || isInitializing) {
		return;
	}
	...
	const host = resolveMapHost();
	if (!host) {
		throw new Error('Map host element is not ready.');
	}
	...
	try {
		...
		mapInstance = new mars3d.Map(host, mergeMapOptions(getDefaultMapOptions(), props.mapOptions));
		isMapCreated = true;

		if (props.useGlobalMap && parentMapContext?.setMap) {
			parentMapContext.setMap(mapInstance);
		}
